...
function RecordAppointments({ email, appointments }) {
  ...
  const handleSearchInputChange = (event) => {
    ...
    const filteredData = rows.filter(
      (row) =>
        row.appointment_schedule.toLowerCase().includes(searchTerm) ||
        row.appointment_schedule_end.toLowerCase().includes(searchTerm) ||
        row.status.toLowerCase().includes(searchTerm) ||
        row.modality.toLowerCase().includes(searchTerm) ||
  ...
  const cols = [
    {
      ...
      renderCell: (params) => (
        <Link
          to={{
            pathname: "/appointment/schedule-management",
            search: `?id=${params.row.linkId}`,
          }}
    ...
    },
    {
      ...
      headerAlign: "center",
      align: "center",
      renderCell: (params) => <span>{dayjs(params.row.appointment_schedule, "DD/MM/YYYY hh:mm:ss a").format("MMM D, YYYY - hh:mm a")}</span>,
    },
    {
      field: "appointment_schedule_end",
      headerName: "To",
      width: 200,
      headerAlign: "center",
      align: "center",
      renderCell: (params) => <span>{dayjs(params.row.appointment_schedule_end, "DD/MM/YYYY hh:mm:ss a").format("MMM D, YYYY - HH:MM a")}</span>,
    },
    {
      field: "status",
      headerName: "Status",
...
export default RecordAppointments;
