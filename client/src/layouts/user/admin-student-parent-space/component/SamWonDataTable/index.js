            ...
            <SoftBox className="relative overflow-x-auto ">
...

                {
                    uniqueArray.map((rowA,ind) => ind >= paginationStart && ind <= paginationEnd ? (
                        <>
                            <SoftBox className="flex flex-row rounded-md w-full border-slate-200 border-b-2 h-32 cursor-pointer select-none"  onClick={() => handleCollapse(ind) }> 
                                <SoftBox
                                    sx={{ width: 100}} 
                                ...
                                </SoftBox>
                                {
                                    ...
                                    null : (
                                        ...
                                        >
                                            <SoftBox
                                                sx={{ width: row["size"] }} 
                                                className={`flex-1 flex ${row["field"] != "name" ? "justify-center" : "justify-start px-24"} items-center flex-shrink-0 text w-full`}
                                            >
                                                <Typography variant="body2" className={`${ row["field"] == "view" ? "overflow-visible" : "overflow-hidden"} text-ellipsis whitespace-nowrap`}>
                                                    { row["field"] == "view" ? (
                                                        <>
